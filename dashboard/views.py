...
def dashboard(request):
    ...
    f as (select coalesce(sum(case when hlt.ht_year is not null or hlt.pht_year is not null or hlt.dm_year is not null or hlt.pdm_year is not null then 1 end),0) as count from health_management_health hlt inner join health_management_patients pt on hlt.patient_uuid=pt.uuid where 1=1 and hlt.status = 2 and pt.status=2 """+village_name+hlt_date_filter+"""), 
    ...
    percentage_sql= """with a as (select coalesce(sum(case when (hlt.ht_year is not null or hlt.pht_year is not null or hlt.dm_year is not null or hlt.pdm_year is not null) and (dm_source_treatment > 0 or ht_source_treatment > 0 or pdm_source_treatment > 0 or pht_source_treatment > 0) then 1 else 0 end),0) as all_data, 
    coalesce(sum(case when (hlt.ht_year is not null or hlt.pht_year is not null or hlt.dm_year is not null or hlt.pdm_year is not null) and (hlt.dm_source_treatment=1 or hlt.dm_source_treatment=3 or hlt.pdm_source_treatment=1 or hlt.pdm_source_treatment=3 or hlt.ht_source_treatment=1 or hlt.ht_source_treatment=3 or hlt.pht_source_treatment=1 or hlt.pht_source_treatment=3) then 1 else 0 end),0) as not_outside_data 
